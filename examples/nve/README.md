...
For a portable reviewer check, use [`notebooks/LLM105_Allegro_NVE_Colab.ipynb`](../../notebooks/LLM105_Allegro_NVE_Colab.ipynb). The notebook verifies the selected model and inputs, downloads a checksummed LAMMPS Kokkos/CUDA executable for an NVIDIA T4, and runs the same NVE stages on the 76-atom unit cell. With the resources available in Colab, this is a smoke test for the paper-scale HPC stability study.
