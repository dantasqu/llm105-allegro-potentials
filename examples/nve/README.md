...
For a portable reviewer check, use [`notebooks/LLM105_Allegro_NVE_Colab.ipynb`](../../notebooks/LLM105_Allegro_NVE_Colab.ipynb). The notebook verifies the selected model and inputs, builds the historical interface with Kokkos/CUDA, and runs the same NVE stages on the 76-atom unit cell. It does not claim to reproduce the paper-scale trajectory: the replicated 304-atom case was not reliable with this historical wrapper on one Colab GPU/rank.
